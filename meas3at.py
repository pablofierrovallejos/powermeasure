...
today = date.today()
filename_his ="/var/www/html/power3_" + today.strftime("%Y%m%d")+ ".dat"
filename_his
today_ant = today
today_pos = today
fenergiainihoy =0.0
...
while 1: 
	...
	miarchivo_his.close()
	filename_his = "/var/www/html/power3_" + today_pos.strftime("%Y%m%d") + ".dat"
	miarchivo_his = open(filename_his,"a")
    ...
    try:
 	...
 	ploads = {"energyday":tmptote,"power":fpower } 
        r = requests.get("http://192.168.18.205/", timeout=15)
        time.sleep(10)
	line = r.text
        if not line:
            line = " "
        ...
        if len(lindata)==5 :  #asegura que vengan 7 columnas de datos
           ...
           if iesprimera_ejecucion == 1:
	        ...
	        print("Energia hoy: " + str(fenergiainihoy))
                ...
                archivototalenergiahoy = open("/var/www/html/totenergiahoy3.txt","w")
